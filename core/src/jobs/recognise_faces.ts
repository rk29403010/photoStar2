...
import { ARCFACE_DST_POINTS, estimateAffine } from '../math-utils';
...
class FaceRecogniser {
    ...
    async init() {
        ...
        console.log('[DEBUG] Loading recognition model:', usableModelPath);
    ...
    async computeEmbedding(imagePath: string, box: number[], landmarks: { x: number, y: number }[]): Promise<number[] | null> {
        ...
        // Convert normalized box to pixel box
        let x1 = box[0] * iw;
        let y1 = box[1] * ih;
        let x2 = box[2] * iw;
        let y2 = box[3] * ih;

        // Expand slightly for "loose crop" before feeding to ArcFace? 
        // ArcFace expects tight 112x112 centered on organs.
        // Since we don't have affine, let's just do a square crop of the box.
        ...
        const cx = x1 + bw / 2;
        const cy = y1 + bh / 2;
        const size = Math.max(bw, bh) * 1.3; // 1.3 expansion factor
        ...
        const buffer = await image
            ...
            .resize(112, 112, { fit: 'fill' }) // Distort to fit if not square
            .removeAlpha()
            .raw()
            .toBuffer();
        ...
        // Output name often '512' or '683' depending on export.
        // Let's check keys dynamically or assume standard export.
...
export async function runFaceRecognitionJob(
    ...
    // Select faces that need recognition
    // We look for assets with 'face_detection' but NO 'face_recognition' (for that asset? OR per face?)
    // Our schema allows multiple rows per task per asset, or one JSON blob?
    // `db.ts`: "unique per asset and task" is NOT enforced by schema, but logical.
    // `face_detection` stores a JSON array of faces.
    // We should probably store `face_recognition` as a similar JSON array, matching indices?
    // Or better: store embeddings IN the face object?

    // If we modify `face_detection` blob, we lose separation of concerns?
    // User requested "jobs".
    // Let's create a new task 'face_recognition' that contains a list of embeddings corresponding to the detection box list.
    ...
    console.log(`[DEBUG] Recognising faces: Found ${assets.length} assets.`);
    ...
    for (const row of assets) {
        ...
        if (faces.length === 0) {
            // No faces, just mark done with empty list
        ...
        processed++;
        if (processed % 5 === 0) {
            onProgress({ status: 'running', processed, total, current: path.basename(row.original_path) });
        }
    }
