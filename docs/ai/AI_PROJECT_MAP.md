...
- Photo tools are registered from manifests under `src/services/photoEditing/tools/plugins/`; run `pnpm.cmd run photo-tool:generate-registry` then `photo-tool:check-registry`. The generated registry is machine-owned. Unknown persisted tool IDs remain visible as unavailable and their data is preserved. `grayscale` is the representative migrated plug-in; eleven legacy tools use the transitional adapter.
...
- Automatic editing is available as an `Automatic` tool in the editor tool grid. Its `PhotoAutomaticPanel.tsx` uses `usePhotoAutomaticAnalysis.ts` and deterministic metrics in `src/shared/photoEditing/automatic.ts` to offer per-setting, Tune-tool, Crop/Rotate/Focus-tool, and whole-photo suggestions from a 360px luminance/colour/edge sample plus persisted frame, face, subject, and region metadata. Suggestions are reviewable, selected by default, and are added as normal non-destructive items in Layers & changes; it never adds EXIF rotation because the renderer already auto-orients sources. Advanced Tune recipes use `src/shared/photoEditing/tune.ts` for black/white points, shadow/highlight recovery, temperature, and tint at preview and full resolution.
