...
| **database / schema / migrations** | `src/data/dbSchema.ts`, `src/data/db.ts`, `src/data/wp16Migrations.ts` | `src/services/events/` | `tests/core/` |
...
`identityClusterReconciliation.ts` indexes candidate clusters by stable Face membership before applying the mutual unique-best overlap contract; do not restore pairwise all-cluster comparison. `semantic-face-performance-benchmark.cjs` measures candidate-review lookup and reconciliation at the WP16 development/target tiers. `semantic-generation-compaction-benchmark.cjs` measures three-generation vector replacement, protected retention, logical compaction and explicit SQLite reclamation. The presentation and selection benchmark scripts accept `--samples` and report retained heap drift for repeated-operation soaks. Exact active-vector retrieval remains a separate measured index follow-up.
