...
Each task capsule may record a short objective, acceptance criteria, and one
delivery phase: `explore`, `build`, or `harden`. These fields are the durable
task card; detailed leaf briefs are generated by the coordinator only when they
are actionable. See `docs/ai/feature-delivery-playbook.md`.

- **Explore** supports conversational prototypes and explicitly bounded mocks.
  Use `qa:quick` and a task-owned runtime when visual evidence is needed.
- **Build** turns the accepted prototype into disjoint, narrow leaf briefs.
  Low-reasoning agents receive only fully-guided mechanical work; an integration
  task owns shared contracts, hosts, registries, workflow tooling, and docs.
- **Harden** removes or approves each mock boundary, adds targeted coverage,
  gathers runtime evidence, then runs `qa:ready` and `qa:merge`.

`ui:smoke` is a focused browser boot check. Readiness and merge invoke it only
when the branch changes UI, web-entrypoint, runtime-boot, or smoke files. It is
outside `qa:quick` so ordinary code edits stay fast.
