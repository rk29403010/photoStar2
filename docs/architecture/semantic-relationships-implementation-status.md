...
- WP16b: the initial 10k-asset exact-copy page measured 291.2 ms p95. Numbered migration `20260913_002_exact_copy_presentation_hash_index` adds the existing query's `assets(file_hash)` access path; the same fixture then measured 119.0 ms p95 (2.6 MiB SQLite), below the development target. A rebuildable exact-copy cache, invalidated by relevant Asset changes and atomically refreshed on the next read, replaces full-table window ranking for normal pages. Its target-tier warm page is 54.1 ms p95 for 100k assets (22.9 MiB SQLite), below the 150 ms interaction target; page offset 10,000 is 91.9 ms p95 and the one-off rebuild is 1131.0 ms. At stretch tier (500k Assets), offset 50,000 is 993.8 ms p95 and cache materialization 6187.4 ms (115.2 MiB SQLite), so further paging optimization is still required. Capture-sequence expansion and bulk-selection measurements remain open.
