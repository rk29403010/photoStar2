...
- WP16b: the initial 10k-asset exact-copy page measured 291.2 ms p95. Numbered migration `20260913_002_exact_copy_presentation_hash_index` adds the existing query's `assets(file_hash)` access path; the same fixture now measures 119.0 ms p95 (2.6 MiB SQLite), below the development target. At the target tier, 100k assets measure 1123.6 ms p95 (19.3 MiB SQLite), still far above 150 ms. The remediation decision is a rebuildable, paged exact-copy presentation projection; target/stretch paging plus capture-sequence expansion and bulk-selection measurements remain open.
...
1. Replace full-table exact-copy window ranking with a rebuildable paged presentation projection, then repeat development and target measurements.
