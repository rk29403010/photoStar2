...
if [ -n "$STAGED_FILES" ]; then
  ...
  if ! node scripts/lint-changed-files.mjs --files "$CODEX_FILE_LIST" --fix --restage; then
    echo "pre-commit: staged autofix failed"
    exit 1
  fi

  if ! node scripts/lint-changed-files.mjs --files "$CODEX_FILE_LIST"; then
    echo "pre-commit: staged lint failed"
    exit 1
  fi

  if ! node scripts/complexity-changed-files.mjs --files "$CODEX_FILE_LIST"; then
