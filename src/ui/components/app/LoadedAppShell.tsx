...
import type { SimilarityOrbit } from '@contracts/core';
...
function isIngestJob(job: BackgroundJob): boolean {
...
function toPresentationOrbitCompatibility(expansion: LibraryPresentationExpansion): SimilarityOrbit {
    return {
...
export function LoadedAppShell(props: LoadedAppShellProps) {
    ...
    const getGalleryPresentationOrbit = useCallback(async (presentationKey: string) => {
        const expansion = await actions.getPresentationExpansion(presentationKey);
        return toPresentationOrbitCompatibility(expansion);
    }, [actions]);
    const setGalleryPresentationCover = useCallback((presentationKey: string, assetId: string) => {
        return actions.setPresentationCover(presentationKey, assetId);
    }, [actions]);
