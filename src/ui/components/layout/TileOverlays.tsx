...
const SensitivityBadgeView: React.FC<{ badge: SensitivityBadge | null }> = ({ badge }) => {
    ...
    return (
        <div className={`absolute top-1.5 left-1.5 rounded px-1.5 py-0.5 text-xs font-bold tracking-wider z-15 backdrop-blur-sm border select-none pointer-events-none ${colorClass}`}>
            {badge.label}
        </div>
    );
};
...
const SelectionCheckbox: React.FC<{ selected: boolean; visible: boolean }> = ({ selected, visible }) => {
    if (!visible) {return null;}

    return (
        <div
            data-selection-toggle="true"
            className={`absolute top-2.5 left-2.5 w-5 h-5 rounded-full flex items-center justify-center z-18 shadow-md motion-safe:transition-all motion-safe:duration-150 cursor-pointer border ${selected ? 'bg-brand-accent text-white border-brand-accent scale-100' : 'bg-black/40 hover:bg-black/60 text-transparent border-white/60 hover:border-white scale-95 hover:scale-100'}`}
        >
            {selected && (
...
const GroupIdPills: React.FC<{
    ...
    hoveredGroupId?: string | null;
    onHoveredGroupIdChange?: (groupId: string | null) => void;
}> = ({ presentation, show, hoveredGroupId, onHoveredGroupIdChange }) => {
    if (!show || !presentation) {return null;}

    const pills = buildGroupIdPillModels([presentation]);
...
const CaptionOverlay: React.FC<{ show: boolean; caption?: string }> = ({ show, caption }) => {
    ...
    return (
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/85 via-black/60 to-transparent pt-5 p-2 pointer-events-none motion-safe:animate-[fadeIn_0.15s_ease-in_forwards]">
            <p className="m-0 text-xs text-slate-200 leading-snug italic line-clamp-3">{caption}</p>
        </div>
    );
};

const DeclusterButton: React.FC<{ visible: boolean; activeFilter?: LibraryFilter; assetId: string; onUntagAsset?: (assetId: string, personId: string) => void }> = ({ visible, activeFilter, assetId, onUntagAsset }) => {
...
const DebugIntent: React.FC<{ debug: boolean; intent: TileIntent }> = ({ debug, intent }) => {
    ...
    return <div className="absolute bottom-1 left-1 text-xs text-white bg-black/50 px-1 py-0.5 rounded-sm">{intent}</div>;
};
...
export const TileOverlays: React.FC<TileOverlaysProps> = ({
    ...
    const overlayVisibility = getTileOverlayVisibility({ isHovered, isScrollSettled, isImageVisible, showGroupIds, isGroupRepresentative });
