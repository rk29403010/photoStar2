...
export function useAppUiState(getDevRuntimeImpact: () => Promise<DevRuntimeImpact>) {
  ...
  const activeInfoTab = (activeInfoTabRaw === 'profile' || activeInfoTabRaw === 'tags' || activeInfoTabRaw === 'people' || activeInfoTabRaw === 'objects' || activeInfoTabRaw === 'lineage' || activeInfoTabRaw === 'group' || activeInfoTabRaw === 'json' || activeInfoTabRaw === 'ailogs') ? activeInfoTabRaw : 'profile';
