...
export type InfoTab = 'profile' | 'people' | 'objects' | 'lineage' | 'group' | 'json' | 'ailogs';
...
export function useAppUiState(getDevRuntimeImpact: () => Promise<DevRuntimeImpact>) {
  ...
  const activeInfoTab = (activeInfoTabRaw === 'profile' || activeInfoTabRaw === 'people' || activeInfoTabRaw === 'objects' || activeInfoTabRaw === 'lineage' || activeInfoTabRaw === 'group' || activeInfoTabRaw === 'json' || activeInfoTabRaw === 'ailogs') ? activeInfoTabRaw : 'profile';
