...
test('resolver prefers manual field edits and keeps unrelated fields machine-derived', async () => {
    ...
    try {
        ...
        assert.equal(bundle.projection.estimatedDate.display_label, 'Christmas 1968');
        assert.equal(bundle.provenance.caption.sourceKind, 'manual');
        assert.equal(bundle.provenance.caption.sourceId, manualAssertion.id);
        ...
        assert.equal(bundle.provenance.estimatedDate.sourceId, refinedBlockId);
        assert.equal(bundle.evidence.machineBlocks.length, 2);
        assert.equal(bundle.evidence.manualAssertions.length, 1);
        assert.equal(bundle.evidence.manualAssertions[0].id, manualAssertion.id);
        ...
        assert.equal(projectionRow?.caption_source_id, manualAssertion.id);
        ...
        assert.equal(projectionRow?.description_source_id, refinedBlockId);
    } finally {
        dbManager.close();
