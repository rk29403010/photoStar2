...
test('ai metadata studio pack script includes Repomix guidance and the prompt/schema targets', () => {
    ...
    assert.match(source, /src\/services\/workflowRuntime\/modules\/generateAiMetadata\/geminiPrompts\.ts/);
    assert.match(source, /src\/services\/workflowRuntime\/modules\/generateAiMetadata\/geminiResponseSchema\.ts/);
