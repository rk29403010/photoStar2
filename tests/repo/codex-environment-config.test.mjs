...
test('Codex environment setup and Debug action use repo-owned worktree scripts', () => {
    ...
    assert.match(environmentSource, /\[\[actions\]\][\s\S]*name = "Stop Debug"[\s\S]*command = "tooling\/scripts\/repo\/codex-worktree-stop-debug\.cmd"/);
    assert.match(environmentSource, /\[\[actions\]\][\s\S]*name = "Doctor"[\s\S]*command = "tooling\/scripts\/repo\/codex-worktree-doctor\.cmd"/);
});
