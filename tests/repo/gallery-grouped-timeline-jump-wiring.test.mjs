...
test('timeline jump and viewport highlight use grouped timeline state instead of flat section indexes', () => {
    ...
    assert.match(groupedTimelineLayoutSource, /if \(\(groups\[groupIndex\]\?\.rows\.length \?\? 0\) <= 0\) \{return;\}/);
    ...
    assert.match(groupedTimelineLayoutSource, /customScrollParent\.addEventListener\('scroll', handleScroll, \{ passive: true \}\)/);
