...
test('getSingleClickTileAction opens single photo view when the info panel is hidden', async () => {
    ...
    assert.equal(getSingleClickTileAction({ showInfoPanel: false, selectionCount: 2 }), 'open');
});
